...
인수인계 ZIP을 **새 폴더**에 풀고 최상위에서 Node.js 24 이상으로 다음을 실행한다. 이번 연구 재현은 외부 패키지 설치나 네트워크가 필요 없다.

```powershell
node verify-handoff.mjs
```

이 명령은 동봉 파일의 SHA-256, AK 계획의 고정 자료와 이전 소스, AJ까지의 재현 결과를 검증하고 짧은 요약을 출력한다. 성공 기준은 A/SP 34/34, 21개 멤버 조합, 양의 패시브 합계 호환 19/19, AJ 두 개별 후보 실패가 그대로 유지되는 것이다. 실패 가설이 계속 실패하는 것도 검증 대상이다.
...
AK 실측은 기존 `baseline-passive-AK-20260909.json`을 수정하지 않고 새 원문 파일로 추가한다. 이후 새 비교 소스로 진행해 이 인수인계의 해시 잠금을 보존한다. ZIP 무결성 검사는 인수인계 직후의 스냅샷 확인용이며 새 작업으로 파일을 바꾼 뒤에는 동일한 결과를 기대하지 않는다.
...
ZIP 최상위의 `session-records/`는 이 작업에서 내보낸 문서·JSON의 보존본이다. 중복 원문을 추가 관측으로 세지 않는다. `provenance/`에는 최초 동기화 기록과 원본 대비 변경 목록이 있다. 프로젝트의 브라우저 localStorage 전체나 설치 런타임은 별도이며, 연구 재현에 필요한 카드·성장·보드 근거는 동봉돼 있다. 최초 데스크톱 폴더는 이 작업의 현재 작업본과 다르므로 전송은 이번 ZIP을 기준으로 한다.
