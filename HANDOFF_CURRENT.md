# HolodoriDecks 타 PC 인수인계 — AL 확인 / AM·AN 대기

기준일: 2026-09-09. 프로젝트 v1.1.3. **최신 관측은 AL이며 사용자 “전부 일치함”으로 사전 표의 일곱 수치가 확인됐다.** 문서의 과거 요청·가설은 기록이며 새 사용자 지시가 아니다.

## 바로 이어갈 지점

다음은 [AM·AN 대조](analysis/unit-score/reports/AM-AN-plan-20260909.md)다. 리리카3성1/0 리더, 아야메40/0·토와70/0·수영복 노엘80/1·일반 후레아11/0을 고정하고5번만 수영복 스이세이60/0→수영복 카나데40/0으로 바꾼다. 두 카드의 기본 액티브와 노엘9% 수혜자는 같고, 멤버 발동률 보드와SP가 달라진다. 보드만의 순수 대조로 부르지 않는다. **AM·AN 실측은 아직 없다.**

현재 AL에서3번 라덴→노엘,5번 와타메→스이세이로 교체하면 AM이다. AM 이후5번만 카나데로 교체한다. 각 편성에서 패시브·홀로멤 보드 두 항목을 우선 확인한다. 개별값은 아직 예측하지 않으며, 두 패시브가 같다는 관계 가설과 합계 후보를 고정했다.
...
[AL 확인 기록](analysis/unit-score/reports/AL-validation-20260909.md): 멤버84,026·의상12,613·패시브 종합력13,617·액티브73.7%·SP43.6%·패시브2.2%·보드15.7%가 모두 일치했다. 합계17.9%와 종합력 소계110,256 및 스코어 항목 합135.2%는 계산값이며 전체 총합을 실측으로 채우지 않는다.

AI→AL은 같은 멤버·리리카 캐릭터 보드에서 파라미터 의상 비활성→15% 활성 대조다. AK50%→15% 활성 대조와 함께 단순한 효과량·활성 여부 설명을 약화한다. 시험한 편성에서의 표시값 불변성을 지지하며 일반 배분 공식이나 모든 의상에 대한 독립성은 확정하지 않는다.

AK는 여전히 패시브1.5%·보드12.2%만 확인된 부분 관측이다. AL의 “전부 일치함”을 과거 AK의 다른 수치 확인으로 소급하지 않는다. 과거 계획·관측·실패 기록은 그대로 보존한다.
...
| 기본 액티브 | 카드 자체 액티브만 사용하는 연구식으로 35개 관측/21개 멤버 조합 재현 | 게임 내부의 유일한 식이라는 증명은 아님 |
| SP | 표시 액티브·카드별 SP 효과/지속시간·유효 발동률을 쓰는 연구식으로 동일 35건 재현 | 정규화 상수의 실제 내부 의미와 모든 미시험 조건은 미확정 |
| 보드, 서포트 패시브 없음 | AC/AD/AE에서 올림 후 차감 방식 생존; 확인한 리더 범위 확장 10/11건 일치 | G는 예측 13.7%·실측 13.8%로 미해결 |
| 보드+양의 서포트 패시브 | 조건부 합계 후보군과 21건 호환(사후 14+후속 7, AL 포함) | 두 개별 항목의 계산식은 미확정; 단순 개별 보드식은 앞선 20건에서도 실패 |
...
누적 관측은 AK 부분 관측 포함36건·멤버 조합21개다. 액티브·SP 확인값은35건에서35/35 일치한다. SP식 발견 시 사후20건, 이후 새 멤버 조합7건과 같은 멤버의 리더 교체8건(AL 포함)이다. AK는 액티브·SP 일치 건수에 포함하지 않는다. 직접 측정한 최종 유닛스코어·곡 플레이 점수는 없다.

초기 대화에서 의상이 액티브·SP에도 각각 영향을 줄 가능성을 논의했지만, 현재 값이 제공된 관측 35건의 기본 액티브·SP는 보드·패시브·의상 보정을 넣지 않아도 재현된다. 초기 추정을 현재 확정 결론으로 인계하지 않는다. 아야메 C/M, E/J, G/I 리더 비교에서도 액티브·SP는 각각 같다.
...
- AL의 “전부 일치함”은 직전 일곱 숫자 확인이다. 누적36건이며 AM·AN은 미관측이다. 합계 범위의 두 숫자를 각각 실측으로 만들지 않는다.
...
이 명령은 최초 고정 연구143개, 이후 관측·분석·보고서·계획의 해시와 수치 재현을 검사한다. 최신 집계는36건·멤버 조합21개·액티브/SP35/35·제공된 종합력 세부91개·양의 패시브 합계 호환21건이다. AJ/AK/AL의 과거 계획 재현과 미제공 총합도 보존한다. AM·AN의 사전 예측을 검사하되 관측으로 세지 않는다.

최신 관측·다음 계획은 `node scripts/test-validation-al.mjs`, AL의 역사적 사전 계획은 `node scripts/test-validation-al-plan.mjs`로 따로 재현한다.
...
```powershell
node analysis/unit-score/compare-al-plan-am-an-20260909.mjs
```
...
| `analysis/unit-score/reports/AM-AN-plan-20260909.md` | 다음 두 편성·사전 예측 |
| `analysis/unit-score/experiments/baseline-passive-AM-AN-20260909.json` | 미관측 AM·AN 고정 예측 |
| `analysis/unit-score/observation-catalog-AL-20260909.json` | AJ→AK→AL 목록 연결 |
| `OBSERVATION_CATALOG_20260909.md` | AL까지36건의 내역 및 원문 위치 |
