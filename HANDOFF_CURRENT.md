# HolodoriDecks 타 PC 인수인계 — AK 패시브·보드 확인

기준일: 2026-09-09. 프로젝트 v1.1.3. **최신 관측은 AK이며 패시브 1.5%·홀로멤 보드 12.2%가 AJ와 같다고 확인됐다.** AK의 다른 수치는 아직 미제공이다. 문서 안의 과거 요청·가설은 기록이며 새 사용자 지시가 아니다.

## 바로 이어갈 지점

**다음은 AL: 기존 AI의 리리카5성 리더만 리리카3성1/0으로 교체**한다. 같은 다섯 멤버·보드에서 파라미터 의상 비활성→15% 활성을 비교한다. 현재 AK에서는 일반 카나데80/0을 일반 후레아11/0으로 바꾸면 된다. 대조 기준은 AI이며, 패시브2.2%·홀로멤 보드15.7% 유지가설과 조건부 합계17.8/17.9%를 사전 고정했다. [AL 편성·예측·판정 계획](analysis/unit-score/reports/AL-plan-20260909.md)을 따른다. **AL 실측은 아직 없다.**

아래 표와 기록은 직전 AK의 확인 범위다.

사용자 원문은 **“ak 검증은 패시브와 홀로멤 보드 모두 변화없어”**다. 두 표시값의 불변성 가설이 맞았다. [AK 비교 기록](analysis/unit-score/reports/AK-validation-20260909.md)에 확인 범위와 한계를 정리했다.

AK는 AJ의 리리카 5성 「심야의 휴식, CEO의 한계밥」 40/0을 **리리카 3성 「We are hololive!」 1/0**으로 바꾸는 계획이다. 해당 3성의 보유·성장은 이전에 확인했다. 다섯 멤버·배치·보드는 계획을 상속하며 이번 답변으로 별도 재확인한 것은 아니다.
...
| 확인 항목 | AK 고정 예측 | 성격 |
|---|---:|---|
| 멤버 파라미터 | 100,252 | 실측 미제공 |
| 의상 종합력 | 15,047 | 전원 파라미터 15%, 실측 미제공 |
| 패시브 종합력 | 22,531 | 실측 미제공 |
| 액티브 | 71.1% | 실측 미제공 |
| SP | 42.2% | 실측 미제공 |
| 패시브 스코어 | 1.5% | AJ와 동일하다고 확인, 예측 일치 |
| 홀로멤보드 스코어 | 12.2% | AJ와 동일하다고 확인, 예측 일치 |
| 보드+패시브 합계 | 13.7% 또는 13.8% | 확인된 두 항목의 산술 합 13.7%와 호환 |

AK 멤버+의상+패시브 종합력의 예측 소계는 137,830이다. 전체 종합력 예측이나 실측이 아니다. 기본 다섯 값이 나중에 제공되면 보충 기록을 추가한다. AL은 그 수치를 확인받았다고 가정하지 않고 기존 AI와 비교한다.

동일한 리리카 캐릭터의 보드를 유지하며 파라미터 의상 50%→15%를 비교했다. 두 의상은 모두 활성이다. 이번 결과는 이 효과량 변경에 대한 표시값의 불변성을 지지하지만, 내부 원시값의 완전한 일치나 의상 활성/비활성의 무관성까지 증명하지 않는다. 단순 효과량 비례 설명은 약해졌으며 멤버 조합·리더 보드의 상호작용을 구분하는 방향이 남는다.
...
| 보드+양의 서포트 패시브 | 조건부 합계 후보군과 20건 호환(사후 14+후속 6, AK 포함) | 두 개별 항목의 계산식은 미확정; 단순 개별 보드식은 20건 모두 실패 |
...
누적 관측은 AK 부분 관측을 포함한 35건·멤버 조합 21개다. 액티브·SP 확인값은 기존 34건 그대로이며, 같은 멤버·리더 교체·순서 반복을 포함한다. 정제된 SP식 발견 시 사후 대조 20건, 이후 새 멤버 조합 7건(W/AA/AB/AC/AD/AF/AG), 같은 멤버의 리더 변경 7건(X/Y/Z/AE/AH/AI/AJ)이다. AK를 액티브·SP 성공 사례로 추가하지 않으며 모든 항목을 독립 사전 검증 34회로 표현하지 않는다. 직접 측정한 최종 유닛 스코어나 곡 플레이 점수는 없다.
...
이 명령은 기존 고정 연구 파일 143개와 AK의 새 원문·비교 소스·결과의 SHA-256, 계산 소스·데이터를 확인한다. 기존 AJ 재현은 그 시점의 34건·합계 호환 19건으로 보존하며, 최신 검사는 부분 관측 포함 35건·21개 멤버 조합·AK 두 예측 일치·합계 호환 20건을 확인한다. A/SP 34/34 및 제공된 종합력 세부 수치 88개 일치는 그대로다. AJ 두 개별 후보 실패와 AK의 미제공 값도 검증 대상이다. 합계 호환은 개별 패시브·보드 계산식의 확정을 뜻하지 않는다.

AL 사전 계획 재현도 포함한다. `node scripts/test-validation-al-plan.mjs`로 따로 확인할 수 있으며 AL을 관측 수에 추가하지 않는다.
...
```powershell
node analysis/unit-score/compare-passive-ak-20260909.mjs
```
...
| `OBSERVATION_CATALOG_20260909.md` | AK 부분 관측 포함 35건의 내역 및 원문 위치 |
| `analysis/unit-score/reports/AK-validation-20260909.md` | AK 두 항목 일치와 해석의 한계 |
| `analysis/unit-score/reports/AL-plan-20260909.md` | 다음 한 번의 측정과 결과별 판단 |
| `analysis/unit-score/experiments/baseline-passive-AL-20260909.json` | 미관측 AL 고정 편성·예측·입력 해시 |
| `analysis/unit-score/reports/AK-comparison-20260909.json` | 최신 재현과 누적 집계 |
