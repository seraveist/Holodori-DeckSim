# HolodoriDecks 타 PC 인수인계 — AO 확인 / AP 대기

기준일: 2026-09-09. 프로젝트 v1.1.3. **AO의 카나데 발동률 보드 해제로 패시브1.7%·보드9.4%가 확인됐다. 다음은 AP 노엘 빈도 대조다.** 문서의 과거 요청·가설은 기록이며 새 사용자 지시가 아니다.

## 바로 이어갈 지점

[AO 결과와 AP 계획](analysis/unit-score/reports/AO-validation-20260909.md)을 따른다. 사용자는 해금한 발동률·빈도 보드를 비용 없이 해제할 수 있다고 확인했다. AN에서 카나데 발동률15%·5%만 끄도록 요청한 AO에 “보드 9.4 패시브 1.7”로 답했다. 두 값은 사전 가설과 일치한다. 다른 보드·카드·SP가 요청대로 유지됐다는 조건에서 발동률 보드의 간접 배분 영향이 확인된다.

새 비례 배분 후보는 양의 패시브24건 중20건에서 두 항목이 모두 맞고, W·AA·AF·AH는 한 항목씩0.1%p가 남는다. 이20건은 모두 관측 후 탐색한 결과이며 독립 검증으로 세지 않는다. AO가 먼저 도착한 뒤 비례식을 찾았으므로 AO도 이 식에 대해서는 사후 재현이다.

**AP는 아직 미관측이다.** AO 편성·카나데 발동률 OFF 상태를 유지하고, 노엘 발동빈도4%만 추가로 끈다. 노엘 발동률 합23.5%, 후레아6%, 리리카 리더 노드는 그대로 둔다.
...
현재200초 모형은 빈도를 끄면 구간 겹침이 바뀌어 합계가 증가한다고 예측한다. 실제 악곡에서 빈도를 낮추는 편이 유리하다는 주장은 아니다. 이 대조는 비례 배분뿐 아니라 빈도 변경의 시간축 모형도 시험한다. 새 식을 서비스에 적용하지 않았다.

## 최신 관측의 확인 범위

AM·AN은 기본 다섯 항목이 모두 일치했으나 기본 액티브·수혜자·리더만으로 패시브 표시가 정해진다는 가설은1.7→1.6%로 탈락했다. AO에서는 같은 카드/SP를 유지한 요청 아래 다시1.7%가 됐다. 발동률 보드를 통제할 수 있게 됐지만 모든 SP 독립성이나 배분 일반식이 증명된 것은 아니다.

AO는 패시브·보드 두 항목만 제공됐다. 액티브·SP·종합력·총합은 예측으로 채우지 않는다. AK도 같은 두 항목만 확인한 부분 관측이며, AL의 총합도 미제공이다. 편성·레벨·다른 노드의 유지 여부는 AO 요청에서 상속하고 별도 감사 완료로 기록하지 않는다.
...
| 보드+양의 서포트 패시브 | 조건부 합계 후보군과 24건 호환(사후 14+후속 10, AO 포함) | 두 개별 항목의 계산식은 미확정; 단순 개별 보드식은 앞선 20건에서도 실패 |
| 패시브·보드 개별 배분 | 새 비례 후보가24건 중20건의 두 항목을 사후 재현 | W·AA·AF·AH 실패. 독립 예측은 AP부터이며 일반식 미확정 |
| 의상·리더 관계 | 초기 아야메 비교에서 패시브·의상 관계를 의심했고, 이후 리더 보드도 함께 통제해야 함을 확인 | 아야메 스코어 서포트 60%의 의상 표시량·패시브 변화 공식, 리리카 판정 보정 기여 미확정 |

누적 관측은 AK·AO 부분 관측 포함39건·멤버 조합23개다. 액티브·SP는 제공된37건에서37/37 일치한다. 두 부분 관측을 새 일치 건수에 포함하지 않는다. 기본 종합력 세부97개와 양의 패시브 합계 후보 호환24건이다. 최종 유닛스코어·곡 플레이 점수는 직접 측정되지 않았다.
...
카드별 가중치 `w = p×효과량×지속시간×floor(200/주기)`에서 `PW=Σ(w×패시브%)`, `BW=Σ(w×100×[(1+발동률%/100)(1+빈도%/100)(1+리더서포트%/100)−1])`를 계산한다. 기존 원시 합계 S를 PW:BW로 나눈 뒤 각0.1 올림한다.24건 중20건에서 두 항목이 일치하지만 W·AA·AF·AH가 남아 서비스 공식으로 채택하지 않았다. 임의 기간405는23/24라도 AF가 실패하며 내부 길이로 확정하지 않는다. 탐색은168개 효과 조합과1,713개 기간·횟수 조합을 기록했다.
...
- AO는 새 관측별 boardOverrides로 카나데 발동률0을 기록했다. 과거 공통 보드 입력을 수정하지 않는다. 누적39건이며 AP는 미관측이다.
...
이 명령은 최초 고정 연구143개와 후속 원문·소스·보고서·계획·스크린샷의 해시 및 수치 재현을 검사한다. 최신 집계는39건·멤버 조합23개·액티브/SP37/37·기본 종합력 세부97개·양의 패시브 합계 후보 호환24건이다. AP는 관측에 넣지 않는다.

최신 AO는 `node scripts/test-validation-ao.mjs`, 새 배분 후보와 미관측 AP 계획은 `node scripts/test-validation-ap-plan.mjs`로 재현한다. 과거 관측·계획 검사도 보존한다.
...
```powershell
node analysis/unit-score/probe-proportional-allocation-20260909.mjs
```
...
| `analysis/unit-score/experiments/baseline-frequency-AP-20260909.json` | 미관측 AP 고정 예측 |
...
| `OBSERVATION_CATALOG_20260909.md` | AO까지39건의 내역 및 원문 위치 |
