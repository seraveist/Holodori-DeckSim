...
- 인게임 상세 실측을 바탕으로 범용 유닛 계산의 액티브·SP와 패시브/홀로멤 보드 배분식을 반영. 200초 정규화 시간창, 발동 확률·효과량의 중간 올림 및 기여 비례 배분을 사용
- 멤버별 발동률·발동빈도와 리더 보드 서포트를 선택 입력으로 처리. 입력하지 않은 계정 효과는 0이며 개인 검증 계정의 수치를 기본값으로 사용하지 않음
- 악곡 미선택 결과는 조합마다 120개 순서를 비교해 공통 110초·800노트·SP 5개 기준의 잠재 점수가 가장 높은 대표 순서 하나만 표시. 선택 악곡은 해당 채보와 선택한 목표로 대표 순서를 결정
- 앱·HTML·Python 패키지 버전을 1.2.0으로 통일하고 자동 버전 일치 검사 추가

### Fixed

- 같은 멤버·능력치의 패시브 배율을 합산한 뒤 올림해 중첩 과대 계산 방지
- 인원 제한 패시브의 대상을 기본 종합력이 높은 적격 멤버부터 선정하고 동일 종합력은 편성 순서로 결정
- 의상·패시브·SP 서포트를 멤버별로 합산하여 중복 곱셈 방지. 서포트 의상 편성의 액티브·SP도 리더를 바꿔도 유지되는 실측값에 맞춤
- 잠재 계산에서 발동 불가 액티브를 성공 처리하거나 SP 확률 증가를 중복 가산하지 않도록 보장

### Validation

- BJ까지 적용 대상 53건 중 패시브/보드 두 항목 동시 일치 52건. 같은 범위에서 액티브와 SP는 제공된 각 49건 모두 일치. 27개 멤버 조합이며 반복·부분 관측을 독립 정확도 표본으로 확대하지 않음
- AX의 보드 계산 10.8% / 실측 10.9% 및 BC·BI 재확인을 알려진 0.1%p 차이로 유지. 카드별 보정 상수는 추가하지 않음
- 스코어 서포트 의상의 의상/패시브 배분은 기존 추정식을 유지. 실제 게임 내부 채보, 최종 유닛스코어와 곡 플레이 점수를 완전히 검증했다는 의미는 아님
- 서비스 엔진 실측 회귀와 해시가 고정된 과거 연구 재현을 분리. 최신 범위·수식·재현 방법은 SCORING_VALIDATION.md와 HANDOFF_CURRENT.md 참조
