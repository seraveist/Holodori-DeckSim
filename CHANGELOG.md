# Changelog

Holodori DeckSim의 공개 릴리스 변경 이력을 기록합니다.

형식은 [Keep a Changelog](https://keepachangelog.com/ko/1.1.0/)의 구성을 참고하고, 버전 번호는 Semantic Versioning을 따릅니다.

## [Unreleased]

### Changed

- 1차 실검증 범위와 남은 오차를 `SCORING_VALIDATION.md`로 정리하고, 결과에 계정 효과 미반영 및 추정값 안내를 한국어·영어·일본어로 표시
- 악곡 미선택 결과도 조합마다 120개 순서를 비교하여 공통 채보의 잠재 점수가 가장 높은 대표 순서 하나만 표시. 조합 중복을 탐색 전에 제거하며 동점은 기존 점수 기준과 카드 ID 순서로 안정적으로 결정
- 110초·800노트·균등한 SP 5개 지점을 배치 비교의 공통 기준으로 사용. 상세에 공통 채보 잠재 점수와 가정한 SP 배치를 표시하고 인게임 비교용 범용 점수 및 실제 악곡의 선택 목표는 유지

### Fixed

- 의상 분리 시 패시브를 리더 서포트가 없는 값으로 고정하던 처리를 수정하여 리더가 적용된 경쟁 상태에서 평가. 실측 E/J의 패시브 2.3%→2.7%로 불변 가설을 기각했으며, 절대값 및 보드 간접 기여는 미보정 상태로 구분
- 실측 G/I에서 리더 의상 서포트가 별도 항목이고 액티브·SP 표시가 유지됨을 확인하여 의상 기여를 액티브·SP에서 분리. 기존 총점과 악곡 계산은 보존하며, 의상 보너스의 절대값 보정은 별도 검증 중
- 범용 유닛 및 Master/Estimated 악곡 계산에서 SP 서포트를 의상·패시브 서포트에 다시 곱하던 오류 수정. 모든 서포트를 멤버별로 합산한 뒤 가장 강한 액티브를 다시 평가하여 기존 Exact 채보 계산과 같은 합산 원칙을 적용
- 같은 멤버·능력치의 패시브 효과를 각각 올림하던 과대 계산을 수정하고, 배율 합산 후 한 번만 올림하도록 변경. 라덴·카나데의 센스 +32% 중첩 실측에서 패시브 종합력이 12,398에서 게임 표시값 12,397로 일치
- 제한 인원 서포트 패시브의 대상을 기본 종합력 순서로 선정하도록 수정. 실측 A/B에서 차이가 없던 라덴·후레아 순서 교환을 계산기도 동일하게 평가하며, 기본 종합력이 같은 경우에는 기존 편성 순서로 결정
- 실측 C에서 단일 능력치 패시브도 기본 종합력 순서로 대상을 선정함을 확인. 수영복 카나데 Lv.40·0개화의 퍼포먼스 +32%가 후레아·와타메에게 적용되도록 수정하여 패시브 종합력 8,172 및 계정 기여 제외 종합력 162,103과 일치

### Validation

- AS3.0/10.1은 비례 배분 후보와 일치했으나 AT3.0/7.7은 같은 후보의7.8 예측에 실패. 두 행 부분 관측과 리더 서포트 덮어쓰기를 추가하고 판정 노드만 분리하는 AU 예측을 고정. 누적44건·액티브/SP40/40, 서비스 공식 유지

- 수정 AR2.4/11.8에서 중간 올림식의 보드11.7 예측 실패. 원금14.0을 둘로 나눠 올려도 실측 합14.2를 만들 수 없어 구조적 기각을 기록. 노드 제약과 AR 계획 수정,42건·액티브/SP40/40·종합력 세부106개 및 다음 AS 기준점을 보존

- AP의 비례 배분 사전 예측2.3/12.1 일치와 AQ의 기존 예측2.4/12.0 실패(실측2.3/12.0)를 기록. 평균 발동 횟수와 보정 전후 액티브의0.1 올림 후 차감 후보가 양의 패시브26건 모두를 사후 재현하며, AR2.5/11.4의 독립 예측을 고정. 누적41건·액티브/SP39/39·기본 종합력 세부103개이며 서비스 공식은 유지

- AO 카나데 발동률 보드 해제 대조 확인(P1.7/B9.4). 새 비례 배분 후보의24건 중20건 사후 일치와 W·AA·AF·AH 실패를 보존하고, AP 노엘 빈도4% 해제 예측(P2.3/B12.1)을 고정. 관측39건·액티브/SP37건이며 서비스 공식은 유지

- AM·AN 스크린샷 원본과 사전 예측 비교 추가: 기본 다섯 항목 일치, 패시브1.7→1.6%로 불변 가설 탈락. 누적38건·액티브/SP37/37·종합력 세부97개·조건부 합계 호환23건과 재현 검사를 갱신하고 서비스 공식 및 과거 예측은 보존

- AL “전부 일치함”을 직전 일곱 수치 확인으로 추가: 누적36건·액티브/SP35건·종합력 세부91개·양의 패시브 합계 호환21건. AM·AN의 동일 액티브/수혜자 카드 대조를 사전 고정하고 미관측·미제공 총합을 보존

- AL 미관측 사전 계획 추가: 기존 AI의 멤버·보드를 고정하고 리리카 리더 카드만 바꿔 파라미터 의상 비활성→15% 활성 대조. 개별 표시값 유지가설과 두 값의 합계 후보를 구분하며 재현 검사에 포함

- AK의 패시브 1.5%·홀로멤 보드 12.2% 불변 확인을 원문과 AJ 참조 근거로 추가. 부분 관측 포함 35건, 합계 호환 20건으로 갱신하되 미제공 종합력·액티브·SP는 예측과 구분하고 기존 고정 예측·서비스 공식은 보존

- 다른 PC의 AJ 완료 / AK 미측정 검증 자료를 동기화. 원문 23건을 추가해 총 34건·멤버 조합 21개에서 액티브/SP 연구식을 재현하고, 양의 패시브·보드 합계 호환 19건과 AJ 개별 후보 실패를 보존. 서비스 계산식은 그대로 두며, Git 체크아웃에서도 원문·고정 예측·연구 소스의 해시를 확인하는 검사와 파일 바이트 보존 설정 추가

- 다른 PC에서도 Node.js만으로 11건 실측·대표 순서·200초 액티브 후보를 재현하는 실행 명령과 인수인계 문서 추가. 공통 환경 탐색, 고정 계수 역산, L/M/N의 미관측 사전 예측과 관측 비교 도구를 보존하며 분석 계수는 서비스에 적용하지 않음

- 1차 반영 이후 H(일반 후레아 Lv.1·0개화) 실측을 추가하여 화면 11건·멤버 조합 6개·리더 포함 편성 8개로 확대. 종합력 127,492가 사전 예측과 일치하고 액티브 67.3%는 두 사전 가설 중 조건 충족 가정과 일치. 같은 SP에서도 38.1%→38.4% 변화가 관측되어 액티브·보드와의 상호작용 검증을 유지하며, 서비스 점수식은 변경하지 않음
- 화면 10건·독립 멤버 조합 5개·리더 포함 편성 7개의 입력을 저장소에 보관. CI에서 확인된 종합력과 반복 관측을 검사하고, 미보정 보너스 차이는 정확도 통과로 취급하지 않고 보고
- 실측 K에서 사용자 확인 편성의 E 전체 표시값 재현과 리리카→아야메→리리카의 패시브 2.3%→2.7%→2.3% 복귀를 확인. 리더 복귀 후 계산 상태가 원래 편성과 일치하는 회귀 추가. 패시브·SP 절대값의 미보정 상태는 유지
- 실측 J의 제외 범위 종합력 109,076과 액티브 66.9%·SP 38.1% 사전 예측 일치 확인. 패시브 2.3% 예측은 실제 2.7%와 달라 실패로 기록. 리더 서포트로 경쟁 액티브가 바뀔 때 패시브 기여를 재평가하는 회귀 추가
- 실측 I(아야메 리더)의 제외 범위 종합력 105,597과 G/I 의상 분리 회귀 추가. 서포트 합산 검사에서도 의상 항목을 별도로 확인
- 실측 G의 종합력 154,410 및 서포트 패시브 비활성 회귀 추가. 액티브 사전 예측 70.1%는 일치했지만 서포트 패시브가 없는 편성에서도 SP 오차가 남아 스킬 보너스 절대값 보정을 계속 검증 중
- 악곡 선택의 Exact/Master/Estimated 및 AUTO/Manual 양쪽 목표에서 조합당 대표 순서 한 개, 중복 후보 제거, 120개 순열 최댓값, 목표별 순서 차이를 검증. 브라우저에서도 실제 m0049를 선택하고 전원 프리셋을 적용했을 때 두 목표 모두 결과 한 개만 표시됨을 검사
- 범용 두 계산 목표의 대표 순서, 120개 순열 최댓값, 조합 중복 제거, 멤버 프리셋, 동점 안정성, 최적화 실행 경로 및 입력 보존 회귀 추가
- 의상 20%·패시브 5%·평균 SP 10%의 합산, SP로 가장 강한 액티브가 바뀌는 경우, Unit/집계/Exact의 서포트 합산 회귀 추가
- 인게임 중첩 패시브 관측값과 전체/단일 능력치 혼합·조건 미충족·대상 인원 제한의 회귀 추가
- 실측 A/B 순서 교환과 해당 멤버 5명의 120개 순열, 서포트 대상 자격·동률 처리 회귀 추가. 스킬 보너스의 절대값은 별도 실측 보정 진행 중
- 사용자 확인 레벨을 적용한 실측 C의 멤버 파라미터·의상·패시브 종합력 회귀 추가
- 실측 D(수영복 스이세이 Lv.60·0개화)의 종합력 154,543과 각 구성 항목 회귀 추가. 200초 액티브 참조식의 사전 예측 70.3%는 새 실측과 일치했으나 패시브·SP 보정이 남아 서비스 엔진 연결은 보류
- 실측 E(토와 Lv.70·0개화)의 종합력 156,630과 각 구성 항목 회귀 추가. 액티브 사전 예측 66.9%도 일치. 단일 서포트 패시브의 실측 2.3%를 통해 중복 가산 또는 고정 대상 순위만 바꾸는 보정으로는 남은 오차가 해결되지 않음을 확인
- 실측 E/F의 와타메·수영복 카나데 순서 교환 회귀 추가. 서로 다른 SP 효과·지속시간에도 게임 상세 항목 전체가 동일했으며, 범용 점수에는 특정 악곡의 SP 배치 이득을 적용하지 않는 원칙을 재확인. 스킬 보너스 절대값의 보정과는 구분
- 채보의 SP 5개 지점에서 기대값·잠재값 모두 배치 효과를 반영하되 범용 유닛 점수에는 곡의 위치 효과가 섞이지 않는지 검증. 순수 발동률 증가 SP의 잠재 고점 중복 가산 방지와 잠재 목표의 120순열 탐색도 확인

## [1.1.3] - 2026-08-23

### Added

- ★4/★5 신규 카드 portrait 자동 동기화와 provenance 기록, landscape illustration 검증/repair 경로 추가
- `holosims.net` 실제 배포 SHA·manifest·JSON 경로·카드 WebP를 검증하는 Production smoke 추가

### Changed

- 공식 라이브 서비스 주소를 `https://holosims.net/`으로 전환
- GitHub Pages 배포 SHA를 JS/카드 이미지 revision으로 사용하여 동일 파일명 교체 시 browser/CDN stale cache를 우회
- 현재 Master 문서 기준을 174카드 / 188악곡 / 752채보 / Runtime Exact 699개로 갱신

### Fixed

- GitHub-hosted runner에서 Octo catalog가 HTTP 403을 반환할 때 공개 card-art snapshot을 우선 사용하고 Octo를 fallback으로 유지
- 신규 portrait가 300×300 카드 icon으로 들어오던 asset-class 오류를 가로형 illustration으로 교정하고 기존 잘못된 자동 import도 repair 가능하게 수정
- Pages revision rewrite가 `manifest.json`을 `manifest.js`로 오인해 production 404를 만들던 회귀를 전용 rewrite script와 JSON 보존 테스트로 차단

## [1.1.2] - 2026-08-14

### Fixed

- 계산 A가 취소된 뒤 새 계산 B가 완료한 경우, 늦게 끝난 A가 B의 결과·상태·계산 버튼을 다시 덮어쓸 수 있던 stale completion race 제거
- 계산 입력 변경 시 현재 요청을 session guard에서 즉시 무효화하고 AbortSignal로 Worker와 채보 fetch를 함께 취소
- 취소된 Runtime/Local Exact 요청이 Master fallback으로 계속 진행하지 않고 즉시 종료되도록 chart loader 보강

### Validation

- `A 시작 → 설정 변경 → B 시작/완료 → A 지연 완료` 순서에서 B만 최종 상태를 commit하는 reverse-completion 회귀 추가
- Runtime Exact chart fetch에 계산 AbortSignal이 실제 전달되고 abort 시 stale chart 결과를 반환하지 않는 회귀 추가
- v1.1.1의 카드 준비, 목표 분리, 액티브 충돌, Exact pruning, Beam, Worker watchdog, 브라우저 smoke 회귀를 그대로 재실행

## [1.1.1] - 2026-08-14

### Changed

- 카드 준비 로직이 `CardPotential`과 `master_refs`의 구조화된 Master 효과/트리거를 우선 사용하도록 분리
- ★4/★5 5개화의 스킬트리 연계 효과를 현재 점수 범위 밖으로 명시적으로 분류
- 최고 유닛 스코어 / 최고 잠재 스코어를 결과 렌더러가 직접 처리하도록 레거시 P/T/S target adapter 제거
- 계산 중 설정이 바뀐 경우 이전 Worker 결과를 폐기하도록 stale-result 방지 추가
- Worker 무응답 시 동기 계산으로 복귀하는 watchdog 추가

### Validation

- ★4/★5 전체 카드의 0~5개화 Master/기존 계산 parity 회귀
- 기대값 최적 편성과 잠재값 최적 편성이 실제로 분리되는 목표 회귀
- 동일 액티브 주기를 hard-ban하지 않고 최종 점수로 판단하는 soft-constraint 회귀
- 252개 조합에서 실제 30개 shortlist pruning 후 `모든 조합 × 5!` 전수 결과와 비교
- 강제 Beam Search와 완전탐색의 synthetic parity 회귀
- 브라우저에서 ★4/★5 정책, TOP 5, Runtime Exact Range 206, Master fail-soft fallback까지 자동 smoke 검증

## [1.1.0] - 2026-08-13

### Added

- 현재 Master 728개 채보 중 703개와 호환되는 Runtime Exact range index
- 고정 공개 chart snapshot에서 선택한 채보 객체만 HTTP Range로 lazy-load하는 Exact timeline 경로
- Local Exact → Runtime Exact → Master → Estimated 순서의 fail-soft 채보 정확도 계층
- 대상 지정 Passive Active-Skill-Effect-Up을 해당 멤버별로 적용하는 계산 경로
- Exact 타임라인의 실제 액티브 판정 시각·확률·combo·SP 발동률·대상 support를 결과 진단에 재사용
- 채보 밀도와 보유 카드 수에 따른 동적 Exact shortlist
- 작은 카드 pool에서 `모든 조합 × 5! 순열` 완전탐색과 staged optimizer 결과를 비교하는 global-search regression
- Runtime Exact index의 current-Master coherence regression
- Runtime Exact Content-Range / byte length / SHA-256 fail-closed 검증
- module Web Worker 기반 편성 탐색과 Worker 미지원 환경의 동기 fallback
- 대상 패시브, 글로벌 Exact 탐색, Runtime coherence에 대한 JavaScript regression tests

### Changed

- Exact 2차 후보를 기존 고정 TOP 10에서 상황에 따라 약 12~30개까지 확대
- 작은 보유 카드 pool에서는 가능한 전체 조합을 Exact 2차 평가 대상으로 보존
- 대상 지정 액티브 효과 지원을 `대상 수 / 5` 팀 평균 근사 대신 실제 대상 멤버의 액티브에 적용
- Exact 결과의 발동 횟수·커버율·팀 타임라인이 집계 재계산이 아니라 실제 타임라인 판정 데이터를 사용하도록 변경
- `Manual FC` 표기를 실제 계산 의미에 맞춰 `Manual PERFECT FC`로 명확화
- Runtime Exact index가 Master 데이터 동기화 시 새 chart index에 맞춰 함께 재생성되도록 sync pipeline 확장
- Runtime source가 새 Master와 불일치하면 해당 채보만 자동 제외하고 Master fallback 유지
- 저장소에 703개 변환 timeline JSON을 bulk-publish하지 않고 range metadata만 유지
- 브라우저의 무거운 조합/순열 탐색을 메인 UI 스레드에서 Web Worker로 이동

### Validation

- Unit Score calibration fixture drift 검사
- 프리셋 멤버 포함 조건 및 120순열 회귀
- 대상 지정 Passive Support 회귀
- Exact staged/global exhaustive parity 회귀
- Runtime Exact current-Master coherence 및 무결성 검사
- PR / Pages 단계에서 신규 계산 regression 재실행

## [1.0.0] - 2026-08-13

### Added

- 보유 카드·레벨·개화 상태 기반 6인 라이브 편성 시뮬레이터
- 리더 1명 + 멤버 5명의 프리셋 조건과 자동 TOP 5 추천
- 최고 유닛 스코어 / 최고 잠재 스코어 목표
- 카드 레벨, 개화, 리더, 액티브, 패시브, 스페셜 스킬 계산
- 악곡·난이도·AUTO/Manual FC별 점수 projection
- Master 풀콤보 노트 수와 note/combo score rules
- Exact SUS metadata의 노트 타임라인·SP 슬롯 시각 계산
- Exact 채보에서 멤버 5명 전체의 SP1~SP5 순서 최적화
- 계산 상세, 스킬 진단표, 개인/팀 발동 타임라인
- 내 보유 카드 관리와 JSON 내보내기/가져오기
- 카드 상세 정보, 검색, 필터, 정렬
- 한국어 / English / 日本語 로컬라이징
- 라이트 / 다크 테마 및 반응형 PC/모바일 UI
- Pretendard Variable 기반 공통 typography 계층
- HolodoriDB Master 자동 동기화와 검증 PR 파이프라인
- Exact chart hash/note-count stale protection
- GitHub Pages 자동 배포

### Changed

- 멤버 프리셋을 슬롯 위치 고정이 아닌 필수 포함 조건으로 변경
- Exact SP 채보에서는 프리셋 출신 멤버를 포함한 최종 5인 전체 순서를 재탐색
- 조합 탐색의 곡 context/kernel 캐시와 목표별 경량 평가 적용
- Exact 순열 평가에서 조합 불변 계산 재사용
- 노트 타임라인 평가를 sweep 방식으로 최적화
- 동일 벤치마크에서 조합+순서 탐색의 반복 계산 비용을 대폭 절감
- 프리셋·선택 팝업·보유 목록·결과 카드의 텍스트 계층 통일
- 결과 카드 리더 배지 높이를 일반 메타 행과 동일하게 정렬

### Infrastructure

- KO/EN/JA를 동일 `master_version` snapshot으로 정렬하는 sync resolver
- generated data 참조 무결성, 급격한 데이터 감소, 언어팩, 채보 hash 검증
- Python sync unit tests와 JavaScript chart/SP regression tests
- PR 검증 → Pages 배포 → VERSION 기반 GitHub Release의 공개 릴리스 흐름
