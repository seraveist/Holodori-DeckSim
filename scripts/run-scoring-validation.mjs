...
for (const script of tests) {
  ...
  const result = spawnSync(process.execPath, [script], { cwd: root, encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
...
const verification = spawnSync(process.execPath, ['scripts/test-research-reproduction.mjs',
  ...(args.includes('--research-grid') ? ['--grid'] : [])], { cwd: root, stdio: 'inherit' });
if (verification.error || verification.status !== 0) throw verification.error ?? new Error('Research reproduction failed');
console.log('[scoring-validation] OK. Known bonus differences remain; generated research output is in .local/scoring-validation/research.');
